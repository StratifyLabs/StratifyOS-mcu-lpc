...
static void exec_writecallback(int port, LPC_UART_Type *uart_regs,
                               u32 o_events);

DEVFS_MCU_DRIVER_IOCTL_FUNCTION(uart, UART_VERSION, UART_IOC_IDENT_CHAR,
                                I_MCU_TOTAL + I_UART_TOTAL, mcu_uart_get,
                                mcu_uart_put, mcu_uart_flush)
...
static void exec_readcallback(int port, LPC_UART_Type *uart_regs,
                              u32 o_events) {
...
int mcu_uart_setaction(const devfs_handle_t *handle, void *ctl) {
  ...
  if (action->handler.callback == 0) {
    ...
    if (action->o_events & MCU_EVENT_FLAG_DATA_READY) {
      ...
      uart_local[port].rx_bufp =
          NULL; // the callback will need to read the incoming data
    }
...
void mcu_uart_isr(int port) {
  ...
  if (uart_regs->LSR & ULSR_RDR) { // this is the RX FIFO ready flag -- read all
                                   // the bytes in the FIFO before exiting
    // todo check for errors with this byte

    if (uart_local[port].rx_bufp != NULL) {
      read_rx_data(port);
      *(uart_local[port].rx_nbyte) =
          *(uart_local[port].rx_nbyte) - uart_local[port].rx_len;
      uart_local[port].rx_bufp = NULL;
    }
  ...
  if (((uart_regs->LSR & ULSR_THRE)) &&
      (uart_regs->IER & UIER_ETBEI)) { // THR is empty and interrupt enabled
...
void mcu_core_uart0_isr() { mcu_uart_isr(0); }

#if MCU_UART_PORTS > 1
void mcu_core_uart1_isr() { mcu_uart_isr(1); }
#endif

#if MCU_UART_PORTS > 2
void mcu_core_uart2_isr() { mcu_uart_isr(2); }
#endif

#if MCU_UART_PORTS > 3
void mcu_core_uart3_isr() { mcu_uart_isr(3); }
#endif
