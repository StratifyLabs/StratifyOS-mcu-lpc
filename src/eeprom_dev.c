...
static int calc_offset(int loc) { return loc % MCU_EEPROM_PAGE_SIZE; }

static int calc_page(int loc) { return loc / MCU_EEPROM_PAGE_SIZE; }

DEVFS_MCU_DRIVER_IOCTL_FUNCTION_MIN(eeprom, EEPROM_VERSION,
                                    EEPROM_IOC_IDENT_CHAR)
...
int mcu_eeprom_setattr(const devfs_handle_t *handle, void *ctl) { return 0; }
...
int mcu_eeprom_write(const devfs_handle_t *handle, devfs_async_t *wop) {
  ...
  while ((eeprom_local[port].offset < MCU_EEPROM_PAGE_SIZE) &&
         (eeprom_local[port].len > 1)) {
...
void write_eeprom_byte(int port) {
  ...
  if ((eeprom_local[port].len == 1) ||
      ((eeprom_local[port].len > 0) &&
       ((eeprom_local[port].offset & 0x01) != 0))) {
    regs->ADDR = eeprom_local[port].offset | eeprom_local[port].page << 6;
