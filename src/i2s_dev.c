...
LPC_I2S_Type *i2s_get_regs(int port) { return i2s_regs_table[port]; }
...
DEVFS_MCU_DRIVER_IOCTL_FUNCTION(i2s, I2S_VERSION, I2S_IOC_IDENT_CHAR,
                                I_MCU_TOTAL + I_I2S_TOTAL, mcu_i2s_mute,
                                mcu_i2s_unmute)
...
int mcu_i2s_getinfo(const devfs_handle_t *handle, void *ctl) {
  ...
  info->o_flags = I2S_FLAG_SET_MASTER | I2S_FLAG_IS_STEREO | I2S_FLAG_IS_MONO |
                  I2S_FLAG_IS_WIDTH_8 | I2S_FLAG_IS_WIDTH_16 |
                  I2S_FLAG_IS_WIDTH_24 | I2S_FLAG_IS_WIDTH_32 |
                  I2S_FLAG_SET_SLAVE | I2S_FLAG_IS_TRANSMITTER |
                  I2S_FLAG_IS_RECEIVER | I2S_FLAG_IS_MCK_ENABLED;
...
int mcu_i2s_setattr(const devfs_handle_t *handle, void *ctl) {
  ...
  // \todo Add pin assign 1 that uses RX WS and RX CLK -- RX WS can be 0.5 or
  // 0.24
...
int mcu_i2s_write(const devfs_handle_t *cfg, devfs_async_t *wop) {
  ...
  write_tx_data(port);
  i2s_regs->IRQ |= (1 << 1); // enable TX interrupt -- interrupt fires when FIFO
...
int mcu_i2s_read(const devfs_handle_t *cfg, devfs_async_t *rop) {
  ...
  if (i2s_local[port]
          .rx.handler.callback) { // is receive interrupt already enabled -
                                  // another context is using the IRQ
    return SYSFS_SET_RETURN(EBUSY);
  }
  ...
  // for non-blocking operations, return number of bytes read or -1 if nothing
  // is available
  if (rop->flags & O_NONBLOCK) {
    ...
    // for blocking operations wait until the entire buffer is read then call
    // the callback
