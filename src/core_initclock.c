...
// int sos_config.sys.core_clock_frequency MCU_SYS_MEM;
...
static void mcu_core_initclock_dev(int fclk, int fosc, u8 clk_src, int pdiv) {
  ...
  psel = 0;
  if (fclk < 80000000) { // fcco must be at least 156MHz -- 80MHz * 2 = 160MHz
                         // below 80 fcco will be at least 80
    psel = 1;
  }
  ...
  } while (MCU_TEST_BIT(LPC_SC->PLL0STAT, 8) ==
           0); // make sure operation was successful
...
static int mcu_core_initclock_dev(int fclk, int fosc, u8 clk_src, int pdiv) {
  ...
  case MAIN_OSC:
    ...
    if (MCU_TEST_BIT(LPC_SC->SCS, OSCSTAT)) {
      ...
      MCU_SET_BIT(LPC_SC->SCS, OSCEN);
      if (fosc > 18000000) { // If sos_config.sys.core_clock_frequency is > 18M set
                             // to high speed oscillator mode
        MCU_SET_BIT(LPC_SC->SCS, OSCRANGE);
      }
    }
    break;
  ...
  if (fclk > fosc) {
    ...
    // sos_config.sys.core_clock_frequency = fosc * 2 * m_mult / clk_div;
  } else if (fclk <= fosc) {
    clk_div = (fosc + fclk / 2) / fclk;
...
// requires mcu_core_osc_freq, sos_config.sys.core_clock_frequency, and
