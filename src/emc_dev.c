...
int mcu_emc_setattr(const devfs_handle_t * handle, void * ctl){
	...
	if( attr->mode & EMC_MODE_DYNAMIC ){
		...
		if( attr->loc == 0 ){
			...
			if( attr->mode & EMC_MODE_USEBLS ){
				mcu_core_set_pinsel_func(mcu_pin(4,29),CORE_PERIPH_EMC,0); //BLS3
			}
		}
	...
	for(i=0; i < width; i++){
		mcu_core_set_pinsel_func(mcu_pin(3,i),CORE_PERIPH_EMC,0);
	}
	...
	for(i=0; i < width; i++){
		mcu_core_set_pinsel_func(mcu_pin(4,i),CORE_PERIPH_EMC,0);
	}

	if( attr->addr_width > 24 ){
		mcu_core_set_pinsel_func(mcu_pin(5,0),CORE_PERIPH_EMC,0);
	}

	if( attr->addr_width > 25 ){
		mcu_core_set_pinsel_func(mcu_pin(5,1),CORE_PERIPH_EMC,0);
	}

	mcu_core_set_pinsel_func(mcu_pin(4,25),CORE_PERIPH_EMC,0); //WE
