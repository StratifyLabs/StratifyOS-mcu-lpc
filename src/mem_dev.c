...
DEVFS_MCU_DRIVER_IOCTL_FUNCTION(mem, MEM_VERSION, MEM_IOC_IDENT_CHAR,
                                I_MCU_TOTAL + I_MEM_TOTAL, mcu_mem_erasepage,
                                mcu_mem_getpageinfo, mcu_mem_writepage)

int mcu_mem_open(const devfs_handle_t *handle) {
  MCU_UNUSED_ARGUMENT(handle);
  return 0;
}
int mcu_mem_close(const devfs_handle_t *handle) {
  MCU_UNUSED_ARGUMENT(handle);
  return 0;
}

int mcu_mem_getsyspage() {
  // this returns the page where the system memory is stored -- for LPC it is
  // the page after SRAM (AHB SRAM)
  return (SRAM_PAGES);
}
...
int mcu_mem_getinfo(const devfs_handle_t *handle, void *ctl) {
  ...
  info->ram_pages = SRAM_PAGES + AHB_SRAM_PAGES;
  info->ram_size = (u32)&_sram_size + (u32)&_ahb_sram_size;
  // info->system_ram_page = SRAM_PAGES;
  info->usage = mcu_ram_usage_table;
  info->usage_size = sizeof(mcu_ram_usage_table);
  return 0;
}
...
int mcu_mem_erasepage(const devfs_handle_t *handle, void *ctl) {
  ...
  if ((page <= last_boot_page) || ((addr >= START_OS) && (addr <= END_OS))) {
...
int mcu_mem_read(const devfs_handle_t *cfg, devfs_async_t *rop) {
  if ((is_flash(rop->loc, rop->nbyte)) || (is_ram(rop->loc, rop->nbyte))) {
