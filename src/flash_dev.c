...
DEVFS_MCU_DRIVER_IOCTL_FUNCTION(flash, FLASH_VERSION, FLASH_IOC_IDENT_CHAR,
                                ...
                                mcu_flash_eraseaddr, mcu_flash_erasepage,
                                mcu_flash_getpage, mcu_flash_getsize,
                                mcu_flash_getpageinfo, mcu_flash_writepage)
...
int mcu_flash_eraseaddr(const devfs_handle_t *handle, void *ctl) {
  ...
  if (((u32)ctl < (u32)&_text) || ((u32)ctl > ((u32)&_etext + data_size))) {
    page = get_page(ctl);
...
int mcu_flash_writepage(const devfs_handle_t *handle, void *ctl) {
  ...
  if (wattr->addr + wattr->nbyte > (u32)&_flash_size) {
    wattr->nbyte =
        (u32)&_flash_size -
        wattr->addr; // update the bytes read to not go past the end of the disk
  }
  ...
  err = mcu_lpc_flash_write_page(get_page((void *)wattr->addr),
                                 (void *)wattr->addr, wattr->buf, nbyte);
  if (err < 0) {
...
int mcu_flash_read(const devfs_handle_t *cfg, devfs_async_t *rop) {
  ...
  if (rop->loc + rop->nbyte > (u32)&_flash_size) {
    rop->nbyte =
        (u32)&_flash_size -
        rop->loc; // update the bytes read to not go past the end of the disk
  }
