...
static void enable_opendrain_pin(const mcu_pin_t *pin,
                                 void *internal_pullup) MCU_PRIV_CODE;
void enable_opendrain_pin(const mcu_pin_t *pin, void *internal_pullup) {
  ...
  pattr.o_flags =
      PIO_FLAG_SET_OUTPUT | PIO_FLAG_IS_OPENDRAIN | (u32)internal_pullup;
...
LPC_I2C_Type *i2c_get_regs(int port) { return i2c_regs_table[port]; }
...
static int i2c_transfer(const devfs_handle_t *handle, int op,
                        devfs_async_t *dop);
...
int mcu_i2c_setattr(const devfs_handle_t *handle, void *ctl) {
  ...
  if (o_flags & (I2C_FLAG_SET_MASTER | I2C_FLAG_SET_SLAVE)) {
    ...
    if (mcu_set_pin_assignment(&(attr->pin_assignment),
                               ...
                               CORE_PERIPH_I2C, port, enable_opendrain_pin, 0,
                               (void *)internal_pullup) < 0) {
      return SYSFS_SET_RETURN(EINVAL);
    }
  ...
  if (o_flags & (I2C_FLAG_PREPARE_PTR_DATA | I2C_FLAG_PREPARE_PTR |
...
int set_slave_attr(const devfs_handle_t *handle, const i2c_attr_t *attr) {
  ...
  // \todo To fix Issue #30, we need to check the dest mem and make sure it is
...
static void mcu_i2c_isr(int port) {
  ...
  case 0x10: // Repeated Start condition
    ...
    if (i2c_local[port].state == I2C_STATE_START) {
      i2c_regs->DAT =
          (i2c_local[port].slave_addr[0].addr8[0] << 1) |
          0x01; // Set the Read bit -- repeated start after write ptr
    } else {
      i2c_regs->DAT = (i2c_local[port].slave_addr[0].addr8[0]
                       << 1); // send the address -- in write mode
    }
  ...
  case 0x18: // SLA+W transmitted -- Ack Received
    if (i2c_local[port].state == I2C_STATE_WR_ONLY) {
      ...
      transmit_byte(i2c_regs, &(i2c_local[port].master),
                    is_no_stop(i2c_local + port));
    } else {
      i2c_regs->DAT =
          i2c_local[port]
              .ptr.ptr8[1]; // Send the offset pointer (MSB of 16 or 8 bit)
      i2c_regs->CONSET = AA;
    }
    break;
  ...
  case 0x28: // Data byte transmitted -- Ack Received
    if (i2c_local[port].state == I2C_STATE_WR_PTR_16_ONLY) {
      i2c_regs->DAT =
          i2c_local[port].ptr.ptr8[0]; // Send the offset pointer (LSB)
      i2c_regs->CONSET = AA;
      i2c_local[port].state = I2C_STATE_WR_PTR_ONLY;
      break;
    } else if (i2c_local[port].state == I2C_STATE_WR_PTR_ONLY) {
      i2c_local[port].state =
          I2C_STATE_MASTER_COMPLETE; // writing the pointer only doesn't support
                                     // I2C_FLAG_IS_NO_STOP
  ...
  case 0x48: // SLA+R has been transmitted; NOT ACK has been received.
    set_master_done(i2c_regs, port, I2C_ERROR_ACK);
    break;
  case 0x30: // Data byte in I2DAT has been transmitted; NOT ACK has been
             // received.
    set_master_done(i2c_regs, port, 0);
    break;
  case 0x38:
  ...
  case 0x00: // Bus error in Master or selected slave mode -- illegal start or
             // stop
    if ((i2c_local[port].state >= I2C_STATE_START) &&
        (i2c_local[port].state <= I2C_STATE_MASTER_COMPLETE)) {
      ...
      i2c_regs->CONSET = STO;
      mcu_execute_event_handler(&(i2c_local[port].slave.handler),
    ...
    // --------------------------------------------------------------------------------
  case 0x68: // Arbitration lost in SLA+R/W as master; Own SLA+W has been
  ...
  case 0x98: // data has been received and NOT ack has been returned on general
             // call address
    receive_slave_byte(i2c_regs, &(i2c_local[port].slave));
    ...
    // the device is no longer addressed and won't interrupt when the stop
    // condition occurs
    mcu_execute_event_handler(&(i2c_local[port].slave.handler),
                              MCU_EVENT_FLAG_DATA_READY, 0);
    break;

  case 0xA8: // Own SLA+R has been received and ack returned
  case 0xB0: // Arbitration lost in SLA+R/W as master; Own SLA+R has been
             // received, ACK has been returned.
  case 0xB8: // Data byte in I2DAT has been transmitted; ACK has been received.
    // load byte and send ack
    i2c_local[port].state =
        I2C_STATE_SLAVE_WRITE; // Slave is being read (host will write the data)
    transmit_slave_byte(i2c_regs, &(i2c_local[port].slave));
    i2c_regs->CONSET = AA;

    break;

  case 0xC0: // Data byte in I2DAT has been transmitted; NOT ACK has been
             // received.
  case 0xC8: // Last data byte in I2DAT has been transmitted (AA = 0); ACK has
             // been received.
    ...
    // condition occurs
    mcu_execute_event_handler(&(i2c_local[port].slave.handler),
                              MCU_EVENT_FLAG_WRITE_COMPLETE, 0);
    break;
  ...
  if (i2c_local[port].state == I2C_STATE_MASTER_COMPLETE) {
    ...
    mcu_execute_event_handler(
        &(i2c_local[port].master.handler),
        MCU_EVENT_FLAG_WRITE_COMPLETE | MCU_EVENT_FLAG_DATA_READY, 0);
  }
...
int i2c_transfer(const devfs_handle_t *handle, int op, devfs_async_t *dop) {
  ...
  i2c_local[port].master.ret = &(dop->result);
